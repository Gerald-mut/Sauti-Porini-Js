    ...
    tools: [
      {
        ...
        description: "Retrieves the latest acoustic sensor classifications from a specific forest sector. Use this to check for illegal logging or poaching sounds.",
        inputSchema: {
          type: "object",
          properties: {
            sector_id: { type: "string", description: "The forest sector ID (e.g., 'SECTOR-7-KAKAMEGA')" }
          },
          required: ["sector_id"]
        }
      },
      {
        ...
        description: "Drafts an emergency SMS for local rangers. RESPONSIBLE AI GUARDRAIL: This only drafts the message to the database for dashboard review. It does NOT send the SMS.",
        inputSchema: {
          type: "object",
          properties: {
            threat_type: { type: "string", description: "Type of threat (e.g., 'Chainsaw', 'Gunshot')" },
            location: { type: "string", description: "Coordinates or sector of the threat" },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "draft_ranger_dispatch") {
    ...
    console.log(`Drafting ${args.urgency} urgency dispatch for ${args.threat_type}...`);
